    ...
    for(auto fname : trainingTextFiles)
    {
        ...
        while(!currAnnotateFile.atEnd())
        {
            ...
            if (!m_DF->m_DS.m_TrainHashTable.contains(currLineId))
            {
                ...
                cv::Mat image = cv::imread(imageFullPath.toStdString(),
                                           CV_LOAD_IMAGE_GRAYSCALE);
                cv::copyMakeBorder(image, image, m_DF->m_params.probDistY,
                                   m_DF->m_params.probDistY, m_DF->m_params.probDistX, m_DF->m_params.probDistX,
                                   cv::BORDER_CONSTANT);
